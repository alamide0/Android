 ...
 *
 */
public class ShowMyMaintainInfos extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		h_header.getImageView().setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				ShowMyMaintainInfos.this.finish();
			}
		});
		h_header.setTxt("本号码维护信息");
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					list = TalkWithInternet.toGetMyMaintainInfos(carnumber,phone, "20");
					
					runOnUiThread(new  Runnable() {
						public void run() {
							if(list==null){
								Toast.makeText(ShowMyMaintainInfos.this, "登陆过期，请重新登陆", 0).show();
								Intent i = new Intent();
	...
	private class MyAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			ViewHolder holder;
			if(convertView==null){
				view = View.inflate(ShowMyMaintainInfos.this, R.layout.item_listview_infos, null);
